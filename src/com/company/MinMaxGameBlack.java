...
//        File file = new File("./src/com/company/board3.txt");
        ...
        try {
            ...
            display(board);
            fancyBoard(board);
            char[] bboard =  swap(board);
            MinMaxGameBlack mm = new MinMaxGameBlack();
            Node res = mm.MaxMinGame(depth,bboard);
            System.out.println("------------------Computer------------------");
            char[] resBoard = swap(res.board);
            display(resBoard);
            fancyBoard(resBoard);
            System.out.println("Positons: "+position_evaluated);
            System.out.println("Res. Estimate: "+res.staticEstimate);

            String outputBoard = String.valueOf(resBoard);
    ...
    public char[] game(char[] bboard, int depth) {              //For Testing only
        MinMaxGame mm = new MinMaxGame();
        Node res = mm.MaxMinGame(depth,bboard);
        return res.board;
    }
    ...
    public Node MaxMinGame(int depth, char[] board) {
        ...
        if(depth == 0){
            int minMaxEstimate = staticEstimationMidEnd(board);
            Node n = new Node();
